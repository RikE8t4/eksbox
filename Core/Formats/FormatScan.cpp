...
gui.LabelArchiveValSet(pLblFormat, false);
...
gui.Status("Detecting Format", 0, 0);
...
string strName = str.ToLower(str.RemoveFileExt(str.ExtractFileName(CItem.OpenFileName)));
strncpy((char*)Name, strName.c_str(), sizeof(Name)); // Doesn't work in BCB

Ext = str.ToLower(str.ExtractFileExt(CItem.OpenFileName));
...
if (memcmp(Header, "WBND", 4) && memcmp(Header, "DNBW", 4))
{
	...
	mOffset = sys.HeaderScan(0, sys.GetFileSize(in, false), "WBND", 4, in);

	// Xwb signature found
	if (mOffset == -1)
	{
		//gui.Status("Scanning For Xwb360 Header", 0, true);
		mOffset = sys.HeaderScan(0, sys.GetFileSize(in, false), "DNBW", 4, in);
		if (mOffset != -1)
		{
			mXwb360 = true;
		}
...
else if (!memcmp(Header, "DNBW", 4)) mXwb360 = true;
...
gui.Status("Recognized Format Found: " + CItem.Format, 0, 0);
return true;
...
//	return false;
//#endif
else if (CItem.Format == "xwb" && !fmt.XwbRead(in, mOffset, mXwb360))
	return false;
