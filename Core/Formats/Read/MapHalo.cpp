...
// Decompress
if (str.ExtractFileExt(CItem.OpenFileName) == "map")
{
	FILE* out = 
	fopen(str.ChangeFileExt(CItem.OpenFileName, ".mapU").c_str(), "wb");
	...
	fclose(in); 
	fclose(out);
	CItem.OpenFileName = str.ChangeFileExt(CItem.OpenFileName, ".mapU");
	gui.Status(CItem.OpenFileName, 300, 0);
	gui.OpenDlgResetFileName();
...
// Show info
gui.SetInfoLbl(1, "Map Index: " + str.IntToStr(*map.index));
gui.SetInfoLbl(2, "Map Index Magic: " + str.IntToStr(*map.indexMagic));
gui.SetInfoLbl(3, "Map Magic: " + str.IntToStr(map.magic));
gui.SetInfoLbl(4, "Total Files: " + str.IntToStr(CItem.Files));
