...
if (CItem.Format == "zwb")
{
	FILE* out = fopen(ChangeFileExt(CItem.OpenFileName, ".xwb").c_str(),"wb");
	...
	fclose(in);
	fclose(out);
	CItem.OpenFileName = ChangeFileExt(CItem.OpenFileName, ".xwb");
...
else if (xwb.x360) BSwapAsm(*xwb.headerSize);
...
else if (xwb.x360) BSwapAsm(*xwb.detailsLength);
...
if (xwb.x360) BSwapAsm(CItem.Files);
...
if (*xwb.ver != 1)
{
	...
	if (xwb.x360) BSwapAsm(*xwb.offset1);
	//else if (*xwb.ver == 41 || *xwb.ver == 43) *xwb.offset1 += 4; // PC
}
else
...
for (int x = 0; x < CItem.Files; ++x, pos += *xwb.detailsLength)
{
	...
	if (*xwb.ver == 1 && !xwb.x360)
	{
		...
		//memcpy(&magic, file.magic, 4);
		//SampleRate.push_back(magic.sampleRate);
//		memcpy(&magicV1, file.magic, 4);
//		SampleRate.push_back(magicV1.sampleRate);
//		if (x==13) {
//		msg.info(magicV1.sampleRate);
//		msg.info(*file.magic);
//		}
//		Channels.push_back(magicV1.channels);
//
//		if (magicV1.format == 0)
//		{
//			AudioFormat.push_back("pcm");
//			if (magicV1.bits == 0) Bits.push_back(8);
//			else Bits.push_back(16);
//		}
//		else
//		{
//			magicV1.format= 1;
//			AudioFormat.push_back("xadpcm");
//			Bits.push_back(4);
//		}
	...
	{
		//memcpy(&xwbMagic360, file.magic, 4);
		//FItem.SampleRate.push_back(xwbMagic360.sampleRate);
		...
		//// Pcm
		//if (magic360.format == 1)
		//{
		//	AudioFormat.push_back("pcm");
		//	if (xwbMagic360.bits == 0)
		//		FItem.Bits.push_back(8);
		// 	else
		//		FItem.Bits.push_back(16);
		//	FItem.Ext.push_back(".wav");
...
if (total > CItem.FileSize || CItem.Files <= 0)
{
	...
	CItem.Files = 1;
	gui.Status("Xwb Hack Mode", sys.Delay(true));
}

//}
//if (startPos !=0 && *xwb.ver == 2 && *xwbfnameDir == 2884)
...
fclose(in);
in =
fopen(ChangeFileExt(CItem.OpenFileName.c_str(),".xsb").c_str(), "rb");
...
//gui.SetInfoLbl(1, "Xwb Version: " + String(*xwb.ver));
