...
if (sortByType)
	for (int x = 0, y = 1; x < siz; ++x, y = 0)
	{
		while (y < siz)
		{
			if (ToUpper(ExtractFileExt(mList[x])) < ToUpper(ExtractFileExt((mList[y]))))
...
else
{
	do{
		...
		{
			mList.push_back(CharToWStr(fd.cFileName));
		}
	}
...
return mDirPath + "\\" + WStrToStr(GetSelectedName()); 
...
for (int i = 0; i < mMaxFilesShow; ++i){
	...
	if (GetSelectedIndex() == i)
		mFont->DrawTextA(WStrToStr(GetItem(i)).c_str(), (int)GetItem(i).length(),
