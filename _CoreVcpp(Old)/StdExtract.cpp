...
inline void ExtractCleanup(FILE* io){
...
if (pFileNum > CItem.Files || pFileNum == 0){ 
	gui.Error("Filenumber Is Invalid", 0, 0);
	return false;
}
...
//++x, ++extracted, ++filesTotal) // && !feof(in)
{
	...
	try{
		...
		{
			fseek(in, FItem.Offset[i], 0);
			CZlib zl; zl.DecompStream(in, out); // Leaking memory!!!!
			ExtractCleanup(out);
			continue;
		}
...
//{
	...
	gui.Status("Finished Processing", 700);
...
//		gui.ClearInfo();
		sys.ClearVariables();
//	}
	gui.FinishDisplay(extracted, files, time, "Saved", "Finished");
